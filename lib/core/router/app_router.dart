...
GoRouter _buildRouter(Ref ref, Listenable refresh) {
  return GoRouter(
    ...
    redirect: (context, st) {
      ...
      // /order/:id (просмотр заказа) гостю доступен — каталог листается без
      // входа. Действия внутри карточки (отклик, контакт, профиль) гейтятся
      // отдельно мягким листом. Поэтому '/order' здесь НЕ перечислен.
